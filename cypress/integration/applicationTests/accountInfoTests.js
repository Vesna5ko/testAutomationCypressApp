...
describe('My Account information suite', function(){
    ...
    it('My account navigation', function(){
        ...
        cy.get('#user-settings-submit').should('be.enabled')
    });
});
...
describe('Bank Account information suite', function(){
    ...
    it.only('Delete created Bank account', function(){
        cy.loginUser('Katharina_Bernier', 's3cret')
        cy.get('[data-test="sidenav-bankaccounts"]').click()
        cy.get('#bankaccount-new').click()
        cy.get('#bankaccount-bankName-input').type('BankName')
        cy.get('#bankaccount-routingNumber-input').type('939393939')
        cy.get('#bankaccount-accountNumber-input').type('939393939')
        cy.contains('Save').click()
        cy.wait(2000)
        cy.get('#bankaccount-list')
          .children()
          .last()
          .find('button', {timeout: 3000})
          .click({ force: true})
        cy.wait(2000)
        cy.get('#bankaccount-list')
            .children()
            .last()
            .find('p')
            .should('have.text', 'BankName (Deleted)')
    });
